...
#define DRIVER_NAME  "gt9xx_gt9157"

struct i2c_client *i2c_client_point = NULL;
static const struct i2c_device_id tpd_i2c_id[] = {{DRIVER_NAME, 0}, {}};
static unsigned short force[] = {0, 0xBA, I2C_CLIENT_END, I2C_CLIENT_END};
static const unsigned short *const forces[] = { force, NULL };
//static struct i2c_client_address_data addr_data = { .forces = forces,};
static struct i2c_board_info __initdata i2c_tpd = { I2C_BOARD_INFO(DRIVER_NAME, (0xBA >> 1))};
...
#if GTP_COMPATIBLE_MODE
    ptr += sprintf(ptr, "Driver VID: 0x%02X%02X\n", gtp_hotknot_calibration_section0[12], gtp_hotknot_calibration_section0[13]);
#else
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    do
    {
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            {  
                #ifdef TGESETURE_APP
                if ((doze_buf[2] == 'c') ||(doze_buf[2] == 'm') || (doze_buf[2] == 's'))
                {
                    GTP_INFO("Wakeup by gesture(%c), light up the screen!", doze_buf[2]);
			gTGesture = doze_buf[2];
...
static void tpd_suspend(struct early_suspend *h)
{
	...
	if(bEnTGesture == 1) 
	{
		GTP_DEBUG(TPD_DC_SYS_RESUME"suspend enable gesture_wakeup.\n");
		#if GTP_GESTURE_WAKEUP
...
static void tpd_resume(struct early_suspend *h)
{
	...
	if(bEnTGesture == 1) 
	{
	GTP_DEBUG(TPD_DC_SYS_RESUME"resume enable gesture_wakeup.\n");
	#if GTP_GESTURE_WAKEUP
		doze_status = DOZE_DISABLED;
		tpd_halt = 0;
	#else
...
int Gt9xx_tpd_get_fw_version( void )
{
    ...
    gtp_read_version(i2c_client_point, &version_info);	
    GTP_INFO("Gt9xx_tpd_get_fw_version lishengli555  : version_info =  %d",
             version_info);  
    return version_info;
}
...
void Gt9xx_tpd_get_fw_vendor_name(char * fw_vendor_name)
{
    ...
    GTP_INFO("Gt9xx_tpd_get_fw_vendor_name lishengli555  : tpd_info.sid =  %d",
             tpd_info.sid);  
    ...
    {
        case 0:
            sprintf(fw_vendor_name, "yeji + %d", cfg_vs);
	      break;
...
#endif
//guomingyi-20141128-add-start.
    .tpd_get_fw_version = Gt9xx_tpd_get_fw_version,
    .tpd_get_fw_vendor_name = Gt9xx_tpd_get_fw_vendor_name,
//guomingyi-20141128-add-end.
};
